...
class AZNet(hk.Module):
    ...
    def __call__(self, x, is_training, test_local_stats):
        ...
        mask = (
            jnp.ones((3, 3, x.shape[-1], self.num_channels), dtype=jnp.bool_)
            .at[0, 0]
            .set(
                False,
            )
        ...
        x = hk.Conv2D(self.num_channels, kernel_shape=3, mask=mask)(x)
        ...
        # policy head
        logits = hk.Conv2D(output_channels=4, kernel_shape=1)(x)
        logits = hk.BatchNorm(
            True,
            True,
            0.9,
        ...
        # value head
        v = hk.Conv2D(output_channels=2, kernel_shape=1)(x)
