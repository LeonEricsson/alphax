...
def search(
    ...
    def body_fun(sim, loop_state):
        ...
        next_node_index = tree.children_index[
            batch_range,
            parent_index, action,
        ]

        next_node_index = jnp.where(
            next_node_index == Tree.UNVISITED,
...
def simulate(
    ...
    def body_fun(state):
        ...
        action = action_selection_fn(
            action_selection_key, tree, node_index,
            state.depth,
        )
        ...
        is_terminal = tree.is_terminal.at[next_node_index].get(
            mode='fill', fill_value=False,
        )
        is_continuing = jnp.logical_and(is_visited, is_before_depth_cutoff)
        is_continuing = jnp.logical_or(is_continuing, is_terminal)
...
def backward(
    ...
    def body_fun(loop_state):
        ...
        leaf_value = reward + \
            tree.children_discounts[parent, action] * leaf_value
        parent_value = (
            tree.node_values[parent] * count + leaf_value
        ) / (count + 1.0)
